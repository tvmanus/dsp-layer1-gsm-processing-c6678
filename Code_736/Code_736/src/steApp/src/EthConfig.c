...
//--------------------------------------------------------------------------
//#include <std.h> // for trs
#include <xdc/std.h>
#include <stdio.h>
//#include <c62.h> // for trs
#include <netmain.h>
...
static void   EthConfig_NetworkOpen();
static void   EthConfig_NetworkClose();
static void   EthConfig_NetworkIPAddr( IPN IPAddr, uint IfIdx, uint fAdd );

// Fun reporting function
static void   EthConfig_ServiceReport( uint Item, uint Status, uint Report, HANDLE hCfgEntry );

// External references
extern int EthUdp_TRxTask();

//---------------------------------------------------------------------------
// Configuration
//
char *HostName    = "tidsp";
char *LocalIPAddr = "10.100.69.241";                  // Set to "0.0.0.0" for DHCP
char *LocalIPMask = "255.255.255.0";    // Not used when using DHCP
char *GatewayIP   = "10.100.69.1";                  // Not used when using DHCP
char *DomainName  = "demo.net";         // Not used when using DHCP
char *DNSServer   = "0.0.0.0";          // Used when set to anything but zero

Task 	oDiagIfd;
static int EthConfig_OpenTask(void); 

//---------------------------------------------------------------------
// Main Entry Point
//---------------------------------------------------------------------
void EthConfig_Init(void)
{
	Task_Init1(&oDiagIfd, "DiagIfd", (pfnTask)EthConfig_OpenTask, NULL, TSK_STACKSIZE_4096, TSK_PRIORITY_5);
}
...
int EthConfig_OpenTask(void)
{
    ...
    else
    {
        ...
        dhcpc.cisargs.pCbSrv = &EthConfig_ServiceReport;
    ...
    do
    {
        rc = NC_NetStart( hCfg, EthConfig_NetworkOpen, EthConfig_NetworkClose, EthConfig_NetworkIPAddr );
    } while( rc > 0 );
...
static void EthConfig_NetworkOpen()
{
    ...
    hHello = DaemonNew( SOCK_DGRAM, 0, 7, EthUdp_TRxTask,
                       2, OS_TASKSTKNORM, 0, 1 );
}
...
static void EthConfig_ServiceReport( uint Item, uint Status, uint Report, HANDLE h )
{
