...
#define GENVECS						//define to save i/o vectors
...
#define OUTPUTFILE_NAME	"..\\..\\3rdparty\\gsmL1\\testApp\\test_gsmL1.txt"
...
for(i=0; i<114;i++)
				...
				fprintf(fp_dump,"normal burst rawbits...\n");
				for(i=0; i<158;i++)
				fprintf(fp_dump, "%d ", assemble_burst_out[i]);
				fprintf(fp_dump,"\n");
				#else
...
void parse_args(int argc, char **argv)
{
	...
	for(i=1; argc > i; i++){
		if( argv[i][0] != '-' ){
			...
			fprintf(fp_dump, "info_bits_length: %d, block_length: %d, multiframe: %d, interleaver_delay: %d, burstType: %d, phaseMode: %d\n",
			info_bits_length, block_length, multiframe, interleaver_delay, burstType, phaseMode);
